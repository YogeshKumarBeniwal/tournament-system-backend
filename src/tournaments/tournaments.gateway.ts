...
export class TournamentsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: SocketWithAuth): Promise<void> {
    ...
    const updatedTournament = await this.tournamentsService.addParticipant({
      username,
      tournamentId,
      userId
    });
    ...
    this.logger.debug(
      `userID: ${username} joined room with name: ${roomName}`,
    );
    this.logger.debug(
      `Total clients connected to room '${roomName}': ${clientCount}`,
    );
    
    this.io.to(roomName).emit('tournament_joined', {userCount: clientCount});

    if(updatedTournament.status == TournamentStatus.IN_PROGRESS){
      this.io.to(roomName).emit('tournament_start', {
        username, 
        userCount: clientCount,
        type: SocketConnectionType.JOIN
      });
    }
  }
  ...
  async handleDisconnect(client: SocketWithAuth): Promise<void> {
    ...
    const { tournamentId, username, userId } = client;
    await this.tournamentsService.removeParticipant({
      username,
      userId,
      tournamentId
    });
    ...
    this.io.to(roomName).emit('tournament_joined', 
      {
        username, 
        userCount: clientCount,
        type: SocketConnectionType.LEAVE
      }
    );
  }
  ...
  async nominate(
    ...
    const updatedTournament = await this.tournamentsService.addParticipantScore({
